...
def main():
    ...
    plot_box(res['data'], labels=parties_list, file_name=f'{param}_box_{short.lower()}.pdf', title=short, number='b', ylabel='fraction of seats', xlabel=f'number of {param}', ylim=(-0.03, 1.03))
    plot_box(res['gall'], labels=parties_list, file_name=f'{param}_box_{short.lower()}_gall.pdf', title=short, number='d', ylabel='Gallagher index', xlabel=f'number of {param}', ylim=(-0.015, 0.35))
    plot_box(res['loos'], labels=parties_list, file_name=f'{param}_box_{short.lower()}_loos.pdf', title=short, number='e', ylabel='Loosemore-Hanby index', xlabel=f'number of {param}', ylim=(-0.015, 0.42))
    plot_box(res['eff'], labels=parties_list, file_name=f'{param}_box_{short.lower()}_eff.pdf', title=short, number='f', ylabel='effective num. of parties', xlabel=f'number of {param}', ylim=(0.93, 10.1))

    res = systems_res['100 districts FPTP']
    short = names_dict['100 districts FPTP']["short"]
    plot_box(res['data'], labels=parties_list, file_name=f'{param}_box_{short.lower()}.pdf', title=short, number='c', ylabel='fraction of seats', xlabel=f'number of {param}', ylim=(-0.03, 1.03))
    plot_box(res['gall'], labels=parties_list, file_name=f'{param}_box_{short.lower()}_gall.pdf', title=short, number='g', ylabel='Gallagher index', xlabel=f'number of {param}', ylim=(-0.015, 0.35))
    plot_box(res['loos'], labels=parties_list, file_name=f'{param}_box_{short.lower()}_loos.pdf', title=short, number='h', ylabel='Loosemore-Hanby index', xlabel=f'number of {param}', ylim=(-0.015, 0.42))
    plot_box(res['eff'], labels=parties_list, file_name=f'{param}_box_{short.lower()}_eff.pdf', title=short, number='i', ylabel='effective num. of parties', xlabel=f'number of {param}', ylim=(0.93, 10.1))
    ...
    for i, ax_col in enumerate(axs):
        for j, ax in enumerate(ax_col):
            ...
            if j != 0:
                ax.set_yticklabels(())
            ax.text(0.5, 6, f'{parties_list[index]} {param}')
            index += 1
