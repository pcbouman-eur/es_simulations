...
def main(arguments=None, input_dir=None):
    ...
    plt.text(0.5, 0.45, r'$\varepsilon =$'+f'{settings["epsilon"]}')
    # plt.legend(loc=1)
    plt.xlim([x_list[0], x_list[-1]])
    plt.ylim([-0.01, 0.54])
    plt.tight_layout()
    plt.savefig(f'plots/pl_sejm_m_sus_gall.pdf')
    plt.close()
    ...
    plt.ylim([-0.01, 0.54])
    plt.tight_layout()
    plt.savefig(f'plots/pl_sejm_m_sus_loos.pdf')
    plt.close()
    ...
    plt.text(-0.15, 2.4, r'$\varepsilon =$'+f'{settings["epsilon"]}', rotation=90)
    plt.legend(loc=1, fontsize=9)
    plt.xlim([x_list[0], x_list[-1]])
    plt.ylim([0.975, 5.02])
    plt.tight_layout()
    plt.savefig(f'plots/pl_sejm_m_sus_eff.pdf')
    plt.close()
