...
def main(arguments=None, input_dir=None, num=None, big=False, num2=None):
    ...
    if big:
        ...
        plt.text(0.5, 0.25, r'$\varepsilon =$' + f'{settings["epsilon"]}')
    ...
    plt.savefig(f'plots/ind_house_otp_m_sus_eps{settings["epsilon"]}.pdf')
    plt.close()
    ...
    if num2 == 'f':
        plt.text(0.06, 7.1, r'$\varepsilon =$' + f'{settings["epsilon"]}')
        plt.ylim([0, 8.5])
    else:
        plt.text(0.55, 3.5, r'$\varepsilon =$' + f'{settings["epsilon"]}')
        plt.ylim([0.5, 4.2])
    ...
    plt.tight_layout()
    plt.savefig(f'plots/ind_house_otp_m_sus_marginal_eps{settings["epsilon"]}.pdf')
    plt.close()
...
if __name__ == '__main__':
    ...
    main(arguments=args, input_dir='results/final1/', num='c', num2='f')  # eps=0.0001

    setattr(args, 'mc_steps', 49)
    main(arguments=args, input_dir='results/final2/', num='b', num2='e')  # eps=0.0007

    setattr(args, 'mc_steps', 51)
    main(arguments=args, input_dir='results/final3/', num='a', big=True, num2='d')  # eps=0.002
